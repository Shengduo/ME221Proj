...
%  Script makes a surface plot of u at the Fourier grid points.
  clc,clear;
  close all;
  N = 1024;      % No. of Fourier modes...should be a power of 2
...
%  Spectral inversion of Laplacian
  ...
  residual_fourier = delsq .* fft2(u) - fhat;
  residual_fourier(1, 1) = 0.;
  residual_fourier = sum(residual_fourier .^ 2 * h^2, 'all');
%  Plot out solution in interior
  disp(strcat("residual = ", num2str(residual)));
  disp(strcat("residual in fourier = ", num2str(residual_fourier)))
  uex = exp(-rsq/(2*sigsq));
